...
for i in features:
    ...
    d.update({f'Product_{count}':{'ID': i.get('data-id') , 'Product_Name':(i.find("p",class_='name')).text,'Price' : (i.find("p",class_='price')).text, 'Available colors' :(i.find("p",class_='colors')).text[18:]}})
    count += 1
print(d)
